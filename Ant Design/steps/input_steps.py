...
def step_fill_by_semantic_label(context, label_text, value):
    ...
    PRIORITY: Tries type first (if label matches common types), then data-attr-id, then semantic matching
    ...
    # PRIORITY 1: Try type matching first (if label matches a common input type)
    if label_lower in input_types:
        print(f"   >> Label '{label_text}' matches input type, trying type matching first...")
        try:
            success = context.input_handler.fill_input(
                label_lower,
                value,
    ...
    # PRIORITY 2: Try data-attr-id (normalize label to data-attr-id format)
    if not success:
